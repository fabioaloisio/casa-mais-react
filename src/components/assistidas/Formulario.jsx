...
import { formatDataForInput } from "../../utils/masks";
...
const Formulario = ({ showModal, setShowModal, onSubmit, assistidaParaEditar, modoEdicao }) => {
    ...
    return (
        ...
        >
            ...
            <Modal.Body>
                <div className="step-indicator mb-4 d-flex justify-content-around">
                    {["Dados Pessoais", "Informações de Contato e Endereço", "História Patológica Regressa I", "História Patológica Regressa II"].map((label, index) => (
                        <div
                            key={index}
                    ...
                    {step === 1 && (
                        <fieldset className='d-flex justify-content-center field'>
                            ...
                            <Row className="d-flex flex-wrap justify-content-center">
                                ...
                                ].map(({ name, label, type = "text", options, mask }, idx) => (
                                    <Col md={3} key={idx}>
                                        <Form.Group>
                        ...
                        <fieldset className="w-100 field d-flex justify-content-center">
                            ...
                            <Row className="w-100 d-flex flex-wrap justify-content-center">
                                {[
                                    { field: "logradouro" },
                                    { field: "bairro" },
                                    { field: "numero", type: "number" },
                                    { field: "cep", mask: "00000-000" },
                                    { field: "estado" },
                                    { field: "cidade" },
                                    { field: "telefone", mask: "(00) 00000-0000" },
                                    { field: "telefone_contato", mask: "(00) 00000-0000" }
                                ].map(({ field, type = "text", mask }, idx) => (
                                    <Col md={field === "cidade" ? 4 : 3} key={idx}>
                                        <Form.Group>
                    ...
                    {step === 3 && (
                        <fieldset className="field d-flex justify-content-center">
                            ...
                            <Row className="w-100">
                                ...
                                { name: "historia_patologica", label: "História Patológica Regressa", as: "textarea" }
                                ].map(({ name, label, type = "text", as }, idx) => (
                                    <Col md={name === "historia_patologica" ? 6 : 3} key={idx}>
                                        <Form.Group>
                                            <Form.Label>{label}</Form.Label>
                                            <Form.Control
                                                name={name}
                                        ...
                                        <Col md={12}>
                                            {formData.quantidade_drogas > 0 && (
                                                <Table bordered className="mt-3">
                                                    <thead>
                                                        <h5 className="mt-4">Uso de Drogas/Bebidas</h5>
                                                        <tr>
                                                            <th>Tipo</th>
                                                            <th>Idade Início</th>
                                                            <th>Tempo de Uso</th>
            ...
            <Modal.Footer>
                ...
                {step < 4 ? (
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            if (validateAllSteps()) {
                                await onSubmit(formData);  // se for async
                                // Swal.fire({
                                //     icon: 'success',
                    ...
                    >
                        {modoEdicao ? 'Atualizar Assistida' : 'Finalizar Cadastro'}
                    </Button>
                )}
            </Modal.Footer>
